...
document.getElementById('search').addEventListener("click",function(){
    ...
    console.log(color)
    let mostSold;
    const most=document.getElementById('mostSold').checked;
    console.log(most)
    if(most)
    {
       mostSold="DESC" 
    ...
    function updateProduct()
    {
        if(productInput.trim()!=='')
        {
            ...
            if(min!=-1||max!=-1)
            {
                url=`/api/filters/?title=${product}&min=${min}&max=${max}&color=${color}&reviews=${cust_reviews}&sortby=${sortby}&mostSold=${mostSold}`
                fetch(url)
